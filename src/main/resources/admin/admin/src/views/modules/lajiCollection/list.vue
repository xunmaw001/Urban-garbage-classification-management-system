        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '垃圾名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.lajiName" placeholder="垃圾名称" clearable></el-input>
                    </el-form-item>
                                                             
                    <el-form-item :label="contents.inputTitle == 1 ? '所属分类' : ''">
                        <el-select v-model="searchForm.lajiTypes" placeholder="请选择所属分类">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                                    v-for="(item,index) in lajiTypesSelectSearch"
                                    v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('lajiCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chengshilajifenleiguanlixitong/upload/lajiCollectionMuBan.xls"
                        >批量导入垃圾收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('lajiCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="lajiCollectionUploadSuccess"
                                :on-error="lajiCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('lajiCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入垃圾收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('lajiCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "lajiCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="lajiPhoto"
                               header-align="center"
                               width="200"
                               label="垃圾照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.lajiPhoto">
                                <img :src="$base.url+scope.row.lajiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="lajiCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.lajiCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('lajiCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('lajiCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('lajiCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    export default {
        ...
        return {
            ...
            userId:"",//当前登录人的id
    //级联表下拉框搜索条件
              lajiTypesSelectSearch : [],
    //当前表下拉框搜索条件
            form:{
                id : null,
                lajiId : null,
                yonghuId : null,
                lajiCollectionTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'laji_collection',//当前表表名,
                        sumColum : 'laji_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'laji_collection_name',//分组字符串字段
                        // types : 'laji_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "laji_collection_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                let params = {
                    ...
                    limit: this.pageSize,
                    sort: 'id',
                    lajiCollectionTypes: 1,
                }

                                         
                if (this.searchForm.lajiName!= '' && this.searchForm.lajiName!= undefined) {
                    params['lajiName'] = '%' + this.searchForm.lajiName + '%'
                }
                                                             
                if (this.searchForm.lajiTypes!= '' && this.searchForm.lajiTypes!= undefined) {
                    params['lajiTypes'] = this.searchForm.lajiTypes
                }
                ...
                params['lajiCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "lajiCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //查询级联表搜索条件所有列表
                this.$http({
                    url: "dictionary/page?dicCode=laji_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.lajiTypesSelectSearch = data.data.list;
                    }
                });
                //查询当前表搜索条件所有列表
            },
            ...
            // 导入功能上传文件成功后调用导入方法
            lajiCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "lajiCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            lajiCollectionUploadError(data){
